...
let splashtext = [
  ...
  "Aassef is interesting...",
]

const runService = async (url, override, overrideadrbar) => {
  if (url.trim() == "") return;
  if (ts.getActiveTab() == null || url == "Aassef://settings" || url == "Aassef:settings" || url == "Aassef:apps" || url== "abyss://apps") {
    openNewtab();
  }
  ...
  if (activeTab) {
    ...
    if (/^(Aassef:)(\/)*[a-z]*/.test(url)) {
      url = url.replace(/^(Aassef:)(\/)*/, "");
      ...
      switch (url) {
        case "newtab":
          ...
          return;
        case "apps":
          const appsPlaceholder = "Aassef apps (Aassef://apps)";
          if (tabElement.querySelectorAll(".extFrame").length > 0) {
            tabElement.querySelectorAll(".extFrame").forEach((frame) => {
              frame.remove();
            });
        ...
        case "settings":
          ...
          if (settingsTemplate instanceof HTMLDivElement) {
            ...
            settingsTemplate.querySelector(".tabTitle").placeholder =
              localStorage.getItem("title") || "Aassef";
